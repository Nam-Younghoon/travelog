...
### 요구사항 명세서
1. 메인페이지 구현
- 회원가입, 로그인, 로그아웃이 가능한 페이지로 이동시키기
- 블로그 입장 시키기  
  
2. 회원가입, 로그인, 로그아웃 구현
- 아이디, 패스워드를 통한 회원가입, 로그인 구현하기

3. 게시글 CRUD 구현
- Create
    - 로그인한 사용자만 사용가능하게 하기
    - 게시글 제목, 내용, 사진 업로드 구현하기
- Read
    - 게시글 제목, 내용을 볼 수 있게하기
    - 조회시, 게시글 조회수를 증가시키기
- Update
    - 로그인한 사용자만 수정하게 하기
    - 내가 작성한 게시글만 수정하게 하기
    - 제목, 내용, 이미지 등 수정할 수 있게하기
- Delete
    - 로그인한 사용자만 삭제하게 하기
    - 내가 작성한 게시글만 삭제하게 하기
4. 회원 비밀번호 변경 및 닉네임 추가 구현
- 회원의 비밀번호를 변경할 수 있게하기
- 회원의 정보를 변경할 수 있게하기
5. 댓글 기능 구현
- 게시글에 댓글을 달 수 있게하기
- 댓글을 삭제할 수 있게하기
- 댓글에 댓글을 달 수 있게하기
6. 정적 파일 모으기
- collectstatic을 이용해 정적 파일을 한 곳으로 모으기
7. AWS에 배포하기
- AWS Lightsail을 이용해 외부에서 접근가능하게 서버 배포하기
...
![기능명세서](https://github.com/Nam-Younghoon/travelog/assets/58909988/64abd84a-e7b0-405f-b780-82bbf3414540)  
...
||게시글 검색|카테고리, 태그, 게시글 제목으로 필터링한 게시글을 조회합니다|남영훈|/blog/search/?q=&sort=|GET|완료|
...
- 한 페이지당 9개의 게시물로 구성되어있으며, 이후의 게시글은 다음 페이지에서 계속됩니다
...
- `Query Parameter`는 `?q=&sort=`로 이루어져 있으며 q에는 검색어, sort에는 정렬조건을 입력받습니다
...
- 이전 게시물, 다음 게시물이 존재할 시 하단에 버튼이 보여져 쉽게 이동 가능합니다
...
## 10. 후기
Django를 직접 사용해볼 수 있는 프로젝트였습니다  
특히 인상깊었던 부분은 Django에는 이미 반복되는 작업들을 구현해놓았다는 것을 몸소 체험해볼 수 있었던 점입니다  
Class Base View를 통해 몇 줄 안되는 코드를 가지고도 CRUD를 가능하게 한다는 게 엄청난 생산성을 가져올 수 있다고 느꼈고, 오히려 너무 간단하게 구현되다보니 이렇게 하는게 맞나? 싶을정도로 기분이 싱숭생숭했습니다  
또한 ORM을 처음 접해보았는데, 테이블에 대한 기본적인 설계만 잘 생각한다면 복잡한 Raw 쿼리 없이도 데이터를 가져올 수 있다는 게 참 편리한 것 같습니다  
해당 프로젝트를 통해서 Django를 통한 기본적인 CRUD 구현방법에 대해 배웠다고 생각합니다